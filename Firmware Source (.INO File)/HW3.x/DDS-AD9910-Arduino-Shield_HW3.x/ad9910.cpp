...
//*** digital Ramp LIMITS enable
  ...
  ////////////////////// TOGLE TRG_OUT
  //delay(10);
  //digitalWrite(TRG_OUT_PIN, HIGH);
  //PORTL |= (1 << PORTL1);

//   delayMicroseconds(10);
//   PORTL &= ~(1 << PORTL1);
// #if DBG==1
//   Serial.println(F("TRG_OUT: LOW"));
//  #endif
  ////////////////////
...
//*** RAM Disable
  ...
  // strBuffer[1] = 0;// RAM_enable; 
  // strBuffer[2] = 0; 
  // strBuffer[3] = Autoclear_digital_ramp_accumulator | Clear_digital_ramp_accumulator;// БЫЛО 0//OSK Disable // Autoclear_digital_ramp_accumulator (нужен чтобы програмно можно был перезапустить DRG c начала);
  // strBuffer[4] = SDIO_input_only ;
  // HAL_SPI_Transmit(&hspi1, (uint8_t*)strBuffer, 5, 1000);
  // HAL_GPIO_WritePin(DDS_SPI_CS_GPIO_PORT, DDS_SPI_CS_PIN, GPIO_PIN_SET);  
  // DDS_UPDATE();
  ...
  //  HAL_Delay(10);
	...
	//  HAL_GPIO_WritePin(DDS_IO_UPDATE_GPIO_PORT, DDS_IO_UPDATE_PIN, GPIO_PIN_RESET); // IO_UPDATE = 0
	 //HAL_Delay(10);
  //Serial.println("Restart DRG");
  // HAL_GPIO_WritePin(DDS_SPI_CS_GPIO_PORT, DDS_SPI_CS_PIN, GPIO_PIN_RESET);    
  // strBuffer[0] = CFR1_addr;
  // strBuffer[1] = 0;// RAM_enable; 
  // strBuffer[2] = 0; 
  // strBuffer[3] = 0; // Autoclear_digital_ramp_accumulator (нужен чтобы програмно можно был перезапустить DRG c начала);
  // strBuffer[4] = SDIO_input_only ;
  // HAL_SPI_Transmit(&hspi1, (uint8_t*)strBuffer, 5, 1000);
  // HAL_GPIO_WritePin(DDS_SPI_CS_GPIO_PORT, DDS_SPI_CS_PIN, GPIO_PIN_SET);  
  // DDS_UPDATE();
